...
3. Compare the whole shortlist on trend (`return20d`), relative strength, realized volatility (`ewmaRealizedVolatility20`), extension (`rangePosition20`), and participation. Promote exactly one finalist, the single best candidate, or none. Do not force a finalist; returning no finalist is the correct answer whenever no setup is clearly strongest.
...
Evaluate each finalist's option surface rather than treating IV as a leg annotation. Use `feed: "indicative"` for option chains and snapshots; this account is not entitled to OPRA and the default feed returns 403, costing the call. Identify indicative data and lower confidence accordingly. Compute target-expiry ATM IV, horizon volatility value, implied move, quote coverage, and the IV difference between the proposed legs. Term structure, skew, and smile curvature are retained when observed but are not required; do not spend calls manufacturing them.

`ivRvVarianceSpread` decides the structure. It is `atmImpliedVolatility^2 - forecastRealizedVolatility^2`: negative means the market prices less movement than your own forecast, so buying premium is the cheaper side; positive means the opposite. Propose a debit vertical only when it is negative and a credit vertical only when it is positive. A structure proposed against its sign is a contradiction, not a judgment call, and the thesis must state the value that justified the choice.

Missing required dimensions are a conflict, not permission to invent them.
...
- In a `PROPOSE_TRADES` report, `ewmaRealizedVolatility20` and `rangePosition20` are required for every shortlisted symbol. `atrPercent20`, `sma20Slope5d`, and `completedSessionDollarVolumeRatio20` are accepted when observed but never required; do not compute them to fill the shape.
...
- Every `optionSurfaces` item requires exactly `{verification, observedAt, underlying, expiration, feed, atmImpliedVolatility, forecastRealizedVolatility, ivRvVarianceSpread, impliedMovePercent, quoteCoverage, eventRisk}` plus `verticalLegIvDifference`, which a proposal also requires, and optional `termStructureSlope`, `putCallSkew25Delta`, and `smileCurvature`. `feed` is uppercase `OPRA`, `INDICATIVE`, or `UNKNOWN`. `eventRisk` is exactly `{verification,status,macroEvents}` plus the schema-applicable optional `eventBeforeExpiration`, `earningsDate`, and `exDividendDate`; do not add temporal, observation, or summary fields inside it.
...
Calculate `aggregateGreeks` as the position-weighted sum: add each `BUY_TO_OPEN` leg and subtract each `SELL_TO_OPEN` leg after multiplying by `ratioQuantity`; set `calculation` to `POSITION_WEIGHTED_SUM`. Bullish net delta must be 0.10 through 0.70; bearish net delta -0.70 through -0.10. A structure whose thesis is not directional must carry absolute net delta no greater than 0.15: a `LONG_STRADDLE` with material net delta is a directional bet wearing a volatility label, and deterministic code rejects it. Compare theta cost, vega exposure, and gamma exposure in a way appropriate to the strategy without inventing hard gamma, theta, or vega limits. These diagnostics integrate risk into research, but deterministic code independently recomputes fresh Greeks and owns approval, sizing, and portfolio capacity.
