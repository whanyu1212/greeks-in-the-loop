...
Each proposal contains `priority`, `direction`, `thesis`, `candidate`, `invalidation`, and `evidence`. The candidate is exactly `{underlying,strategy,legs}` with one through four ordered opening legs, exact OCC symbols, and simplest-form positive ratios. The option types, expirations, strikes, buy/sell intents, and ratios must match the declared strategy's Alpaca shape. Candidate diagnostics share the market observation time and repeat those ordered legs with `contractSymbol`, `positionIntent`, `ratioQuantity`, delta, IV, gamma, theta, vega, volume, open interest, and open-interest date.
