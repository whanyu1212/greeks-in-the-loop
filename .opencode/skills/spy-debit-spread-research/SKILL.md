...
8. **Emit the contract**
   ...
   - For a proposal, report an active account, options trading approval, no conflicting strategy exposure, and a current-cycle `LIVE` market regime whose retained SMA/VWAP metrics support the direction. The application rejects regime observations older than 60 seconds at its pre- and post-quote evaluation boundaries. Include at least one `SOURCED_FACT` with snapshotRef `alpaca-proposal-quotes-v1` for the exact legs.
