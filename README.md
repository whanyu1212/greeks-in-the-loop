...
`buildResearchCyclePrompt` interpolates two `JSON.stringify` payloads into every cycle request: the application-computed `ResearchEligibilityV1`, and the ledger-projected `ResearchContextV1`. Both are generated by application code today, so no model-authored text re-enters a later prompt and the composition carries no injection path. Durable context is already labeled historical planning context only, and the prompt states that all account, market, quote, and freshness facts must be refreshed.

The constraint is forward-looking. If a future field ever carries model-authored text back into a prompt — post-trade reflection and invalidation carry-forward are the proposed cases — it must be clearly delimited from application-generated data and explicitly labeled untrusted, and it must not be permitted to state account, market, quote, or freshness facts. Otherwise prior-cycle model output re-enters as apparent application data, which is a second-order injection path and should be a deliberate decision rather than an accident of schema growth.
